...
class DashboardController extends Controller
{
	...
	public function __invoke()
	{
        ...
        $mp = $m==1?12:$m-1;
        $yp=$m==12?$y-1:$y;
        $achats = Achat::where('fournisseur_id',auth()->user()->fournisseur_id)->where('mois',$m)->where('annee',$y)->where('site_id',auth()->user()->site_id)->get();
        $achats_prev = Achat::where('fournisseur_id',auth()->user()->fournisseur_id)->where('site_id',auth()->user()->site_id)->where('mois',$mp)->where('annee',$yp)->get();
        $cashback_prev = Cashback::where('fournisseur_id',auth()->user()->fournisseur_id)->where('moi_id',date('m'))->where('annee',date('Y'))->first();
		return view('Site/dashboard')->with(compact('achats','cashback_prev','achats_prev'));
	}

    public function getKpi(){
        $achats = Achat::where('annee',date('Y'))->where('fournisseur_id',auth()->user()->fournisseur_id)->where('site_id',auth()->user()->site_id)->get();
